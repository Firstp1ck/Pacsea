...
5. **Caching**:
   ...
   - Partial cache loading for flexible dependency resolution
   - `--clear-cache` CLI argument to clear all cache files (deps, files, services, sandbox, details)
...
7. **PKGBUILD Files**:
   - Added `PKGBUILD-bin` and `PKGBUILD-git` for AUR distribution
   - Both PKGBUILDs install i18n configuration and locale files
   - Locale files are installed to `/usr/share/pacsea/locales/`
   - i18n config installed to `/usr/share/pacsea/config/i18n.yml`

8. **Documentation Updates**:
   - Clarified README instructions for reloading configuration files
   - `theme.conf` reloads with **Ctrl+R**
   - `settings.conf` and `keybinds.conf` are automatically refreshed from disk

9. **Code Quality & Testing**:
   - Added complexity analysis tests (`tests/cyclomatic_complexity.rs`, `tests/data_flow_complexity.rs`)
   - Tests calculate module complexity metrics to help prioritize refactoring efforts
   - Provides insights into code maintainability and refactoring priorities

10. **Version Update**:
    - Updated project version to 0.5.0
    - Reflects the significant feature additions in this release
...
- `src/app/files_cache.rs`: Partial cache loading support
- `src/main.rs`: Added `--clear-cache` CLI argument
- `PKGBUILD-bin`: AUR binary package with i18n support
- `PKGBUILD-git`: AUR git package with i18n support
- `README.md`: Clarified configuration file reloading instructions
- `tests/cyclomatic_complexity.rs`: Complexity analysis test for refactor prioritization
- `tests/data_flow_complexity.rs`: Data flow complexity analysis test
- `Cargo.toml`: Updated version to 0.5.0
...
### Cache Management:
- `--clear-cache` CLI argument clears all cache files and exits
- Clears: `install_deps_cache.json`, `file_cache.json`, `services_cache.json`, `sandbox_cache.json`, `details_cache.json`
- Partial cache loading allows matching caches even when install list signature partially matches
- Enhanced logging for cache operations and preflight processes

### Code Quality Improvements:
- Added complexity analysis tests to identify modules that would benefit from refactoring
- Cyclomatic complexity and data flow complexity metrics help prioritize technical debt reduction
- Tests can be run with `cargo test --test cyclomatic_complexity` and `cargo test --test data_flow_complexity`

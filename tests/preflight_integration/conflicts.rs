...
///
/// Details:
/// - Verifies conflicts are still present after switching tabs
/// - Verifies pacsea-bin's conflicts are still intact
fn verify_conflicts_persist_through_tab_switches(app: &mut crate_root::state::AppState) {
...
/// What: Verify conflicts persist after tab switches.
///
/// Inputs:
/// - `app`: Application state with preflight modal
///
...
///
/// Details:
/// - Verifies conflicts are still present after switching back to Deps tab
fn verify_conflicts_persist(app: &mut crate_root::state::AppState) {
    // Manually sync dependencies without merging (like original test)
    if let crate_root::state::Modal::Preflight {
        items,
        action,
...
/// What: Verify final state of all tabs.
///
/// Inputs:
/// - `app`: Application state with preflight modal
///
...
///
/// Details:
/// - Verifies all tabs have correct data despite conflicts
...
fn preflight_all_tabs_load_correctly_when_conflicts_present() {
    ...
    let _test_packages = setup_test_data_with_conflicts(&mut app);

    // Test 1: Switch to Deps tab - verify conflicts are detected and shown
    verify_deps_tab(&mut app);
...
fn preflight_conflicts_not_overwritten_when_packages_added_sequentially() {
    ...
    // Step 1: Add pacsea-bin first and verify conflicts
    let pacsea_bin = setup_pacsea_bin(&mut app);
    verify_pacsea_bin_conflicts(&mut app);

    // Step 2: Add jujutsu-git
    let _jujutsu_git = add_jujutsu_git(&mut app, &pacsea_bin);

    // Step 3: Verify all conflicts after both packages are added
    verify_all_conflicts_after_both_packages(&mut app);

    // Step 4: Verify conflicts persist through tab switches
    verify_conflicts_persist_through_tab_switches(&mut app);
}

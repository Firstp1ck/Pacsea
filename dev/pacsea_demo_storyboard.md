...
## Part 1 Demo Video — Completed

- [x] Cold open
- [x] Unified search
- [x] Always-visible details pane
- [x] Fuzzy search toggle
- [x] AUR status markings
- [x] PKGBUILD preview and check
- [x] AUR Comments viewer
- [x] Persistent recent searches
- [x] Queue multiple packages
- [x] Preflight modal — deps and review
- [x] Integrated process execution
- [x] Keyboard-first navigation (keyboard-only across all shown parts)
- [x] AUR package voting via SSH
- [x] Secure password prompt
- [x] Update single packages
- [x] Updates and announcements on startup

## Remaining Demo Sections — Not Done Yet

- [ ] **Scene 14 — AUR security scan config**
  - [ ] Open Preflight on an AUR package.
  - [ ] Enter security scan config modal.
  - [ ] Show tool list clearly: ClamAV, Trivy, Semgrep, ShellCheck, VirusTotal, custom patterns, aur-sleuth.
  - [ ] Hold on the tool list for readability before continuing.
- [ ] **Scene 15 — AUR security scan run + summary**
  - [ ] Start scan from the config/preflight flow.
  - [ ] Show per-tool progress in real time.
  - [ ] Wait for final summary screen.
  - [ ] Capture severity breakdown + Semgrep hit count + VirusTotal stats.
- [ ] **Scene 17 — Updates modal walkthrough**
  - [ ] Trigger updates banner/modal.
  - [ ] Show version diff list (official + AUR).
  - [ ] Open Preflight from the updates flow.
- [ ] **Scene 18 — Installed-only mode**
  - [ ] Switch to installed-only mode.
  - [ ] Show leaf-package filter behavior.
  - [ ] Switch to all explicitly installed packages.
- [ ] **Scene 19 — Downgrade workflow**
  - [ ] Select an installed package.
  - [ ] Open downgrade workflow.
  - [ ] Show previous-version list from `downgrade`.
- [ ] **Scene 20 — Distro-aware system update tools**
  - [ ] Open system update dialog.
  - [ ] Show Force Sync (`-Syyu`) option.
  - [ ] Show mirror tool auto-detection (reflector / eos-rankmirrors / cachyos-rate-mirrors).
  - [ ] Show AUR update fallback prompt.
- [ ] **Scene 21 — News feed unified view**
  - [ ] Switch to news mode.
  - [ ] Show mixed feed sources (Arch news, advisories, update notifications, AUR comments).
  - [ ] Demonstrate source filtering.
- [ ] **Scene 22 — News interactions**
  - [ ] Bookmark one advisory/news item.
  - [ ] Mark one item read.
  - [ ] Use news search and show search history dropdown.
- [ ] **Chapter 7 — Config walkthrough (Scenes 24–29)**
  - [ ] Scene 24: layout percentages + pane visibility toggles live.
  - [ ] Scene 25: sort mode switch (`best_matches` -> `aur_popularity`) + startup/installed mode keys.
  - [ ] Scene 26: privilege/auth keys (`privilege_tool`, `auth_mode`, `use_passwordless_sudo`, `skip_preflight`) with safety callout.
  - [ ] Scene 27: scan toggles + `virustotal_api_key` + ShellCheck exclusion list.
  - [ ] Scene 28: mirrors/news/locale/update interval keys (`selected_countries`, `mirror_count`, `news_*`, `locale`, `updates_refresh_interval`).
  - [ ] Scene 29: AUR voting SSH keys + terminal/theme keys.
- [ ] **Scene 30 — Outro**
  - [ ] Cut to clean terminal prompt.
  - [ ] Show install command (`paru -S pacsea-bin`).
  - [ ] Hold GitHub URL long enough for screenshot/readability.
...
> Unified search · Fuzzy mode · AUR status markings
...
| --- | --------------------- | ------ | -------- | ---------------------------------------------------------------------------------------- |
| 1   | Search & Discovery    | `0:00` | ~34s     | Cold open, unified search, details pane, fuzzy, OOD markings                             |

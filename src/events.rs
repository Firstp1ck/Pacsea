...
fn find_in_recent(app: &mut AppState, forward: bool) {
    let Some(pattern) = app.pane_find.clone() else { return };
    ...
    let n = inds.len();
    for _ in 0..n {
        vi = if forward { (vi + 1) % n } else { if vi == 0 { n - 1 } else { vi - 1 } };
        let i = inds[vi];
...
fn find_in_install(app: &mut AppState, forward: bool) {
    ...
    for _ in 0..n {
        ...
        if let Some(p) = app.install_list.get(i) {
            if p.name.to_lowercase().contains(&pattern.to_lowercase()) || p.description.to_lowercase().contains(&pattern.to_lowercase()) {
    ...
    if let CEvent::Key(ke) = ev {
        ...
        if matches!(app.focus, Focus::Recent) {
            ...
            if app.pane_find.is_some() {
                match ke.code {
                    KeyCode::Enter => { find_in_recent(app, true); crate::ui_helpers::trigger_recent_preview(app, preview_tx); }
                    KeyCode::Esc => { app.pane_find = None; }
                    KeyCode::Backspace => { if let Some(buf) = &mut app.pane_find { buf.pop(); } }
                    KeyCode::Char(ch) => { if let Some(buf) = &mut app.pane_find { buf.push(ch); } }
                    _ => {}
                }
                return false;
            }
            match ke.code {
                KeyCode::Char('j') => { // vim down
                    let inds = crate::ui_helpers::filtered_recent_indices(app);
                    if inds.is_empty() { return false; }
                ...
                KeyCode::Char('/') => { app.pane_find = Some(String::new()); }
                KeyCode::Esc => { app.focus = Focus::Search; refresh_selected_details(app, details_tx); }
                KeyCode::Tab => { // Recent -> Search (cycle)
                    app.focus = Focus::Search;
                    refresh_selected_details(app, details_tx);
                }
                KeyCode::BackTab => { app.focus = Focus::Search; refresh_selected_details(app, details_tx); }
                KeyCode::Left => { app.focus = Focus::Search; refresh_selected_details(app, details_tx); }
                KeyCode::Right => { if app.install_state.selected().is_none() && !app.install_list.is_empty() { app.install_state.select(Some(0)); } app.focus = Focus::Install; }
                KeyCode::Down => {
                    let inds = crate::ui_helpers::filtered_recent_indices(app);
                    if inds.is_empty() { return false; }
                    let sel = app.history_state.selected().unwrap_or(0);
                    ...
                    if let Some(vsel) = app.history_state.selected() {
                        ...
                        if let Some(q) = app.recent.get(i).cloned() {
                            ...
                            tokio::spawn(async move {
                                ...
                                if let Some(item) = items.iter().find(|it| it.name.eq_ignore_ascii_case(&q)).cloned() { let _ = tx.send(item); } else { let _ = tx.send(items[0].clone()); }
                            });
                        }
                    }
                }
                KeyCode::Enter => {
                    ...
                    if let Some(vsel) = app.history_state.selected() {
                        let i = inds.get(vsel).copied().unwrap_or(0);
                        if let Some(q) = app.recent.get(i).cloned() {
                            app.input = q; app.focus = Focus::Search; app.last_input_change = std::time::Instant::now(); app.last_saved_value = None; send_query(app, query_tx);
                        }
                    }
                }
                _ => {}
        ...
        if matches!(app.focus, Focus::Install) {
            if ke.code == KeyCode::Char('c') && ke.modifiers.contains(KeyModifiers::CONTROL) { return true; }
            // Pane-search mode first
            if app.pane_find.is_some() {
                match ke.code {
                    KeyCode::Enter => { find_in_install(app, true); }
                    KeyCode::Esc => { app.pane_find = None; }
                    KeyCode::Backspace => { if let Some(buf) = &mut app.pane_find { buf.pop(); } }
                    KeyCode::Char(ch) => { if let Some(buf) = &mut app.pane_find { buf.push(ch); } }
                    _ => {}
            ...
            match ke.code {
                ...
                KeyCode::Tab => { // Install -> Recent (cycle)
                    if app.history_state.selected().is_none() && !app.recent.is_empty() { app.history_state.select(Some(0)); }
                    app.focus = Focus::Recent;
                    crate::ui_helpers::trigger_recent_preview(app, preview_tx);
                }
                KeyCode::BackTab => { app.focus = Focus::Recent; }
                KeyCode::Left => { app.focus = Focus::Recent; }
                KeyCode::Right => { app.focus = Focus::Search; }
                ...
                KeyCode::Delete => {
                    ...
                    if let Some(vsel) = app.install_state.selected() {
                        ...
                        if i < app.install_list.len() {
                            ...
                            if vis_len == 0 { app.install_state.select(None); }
                            else {
        ...
        match (code, modifiers) {
            ...
            (KeyCode::Tab, _) => {
                ...
                if app.install_state.selected().is_none() && !app.install_list.is_empty() { app.install_state.select(Some(0)); }

...
///
/// Inputs:
/// - N/A (compile-time `include_str!` of `config/repos.conf`).
///
/// Output:
/// - Full TOML template string (third-party repo recipes, disabled by default).
///
/// Details:
/// - Single source of truth: edit `config/repos.conf` in the repository; rebuild to refresh the embedded copy.
pub const REPOS_SKELETON_CONTENT: &str = include_str!("../../../config/repos.conf");

...
pub fn render_preflight(
    ...
    // Build header string from spans (for backwards compatibility with existing code)
    // Note: This is only used for calculating tab positions, actual rendering uses tab_spans
    // Extract text content from spans for width calculation
    for span in &tab_spans {
        // Span content is accessible via as_ref()
        header.push_str(span.content.as_ref());
    }

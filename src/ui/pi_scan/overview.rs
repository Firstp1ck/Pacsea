...
fn push_activity_lines(lines: &mut Vec<Line<'static>>, app: &AppState) {
    ...
    lines.push(super::labeled_line(
        ...
        crate::i18n::t(
            app,
            if pi.settings.background_enabled {
...
mod tests {
    ...
    /// Overview exposes the direct Shift+B background scan control and current preference.
    #[test]
    fn overview_renders_background_toggle_control() {
        let backend = TestBackend::new(100, 30);
        let mut terminal = Terminal::new(backend).expect("test terminal");
        let mut app = AppState::default();
        load_english(&mut app);
